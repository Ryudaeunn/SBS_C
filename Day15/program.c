...
void addMenu(ItemList * itemList)
{
   ...
   case CafeLatte:
                ...
                break;
   case Juice:
                addMenuProcess(itemList, "쥬스", Juice_Price);
                break;
   case Icecream:
...
void addMenuProcess(ItemList * itemList, char* menu, int price)
{
    int i;
    printf("%s 추가", menu);
    i = itemList ->indexofFoods;        // 현재 아이템의 index
    itemList->foods[i] = menu;          //선택 목록에 메뉴 추가
    itemList->indexofFoods++;           //총 개수 + 1
...
2. 음료 메뉴 
- 함수 이름 : showMenu()
- 기능 : 주문할 수 있는 메뉴를 출력하는 함수
*/

/*
음료 추가 함수 정의
함수 이름 : addMenu(ItemLIst *itemList)
기능
: 사용자가 추가할 음료를 선택하면,
- ItemList 구조체의 멤버 배열에 해당 음료를 추가
- 음료수 총 개수를 1 증가
...
음료 추가 처리 함수 정의
함수 이름 : addMenuProcess(ItemList*itemList)
기능 : 음료 추가 처리를 하는 함수
